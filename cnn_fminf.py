## tutorial by adesphande3 on github, using fashionMNIST
...
##### Optimization for cost function ####
# param for Adam Grad Descent
learning_rate = 0.001
correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1)) #y_, ?? 
beta1, beta2 = 0.9, 0.99
epsilon = 1e-08
trainStep = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=beta1, beta2=beta2, epsilon=epsilon).minimize(crossEntropyLoss)
...
batchSize = 100
trainAccuracy = 0
for i in range(600): # took about 5min to train with 600 iter and 100 batchSize
