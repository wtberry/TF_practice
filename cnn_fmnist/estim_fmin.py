 ## estimator practice from TF official tutorial, applied on FMNIST dataset
...
X, y= X.astype(np.float64), y.astype(np.int32)
X_test, y_test = fin.data_in('test')
m, n = X.shape
...
def main():
   ...
   ''' 
    ...
    feature_columns = [tf.feature_column.numeric_column("X", shape=[n])] # 784 features in X

    # build 3 layer DNN with 10, 20, 10 units respectively. 
    classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, hidden_units=[10, 20, 10], n_classes=num_labels, model_dir='/home/wataru/machineLearn/kaggle/TF_practice/estim_dir', activation_fn=tf.nn.relu) # model_dir is the dir where TF stores the record data
